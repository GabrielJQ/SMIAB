...
describe('SupabaseAuthGuard', () => {
  ...
  beforeEach(() => {
    ...
    guard = new SupabaseAuthGuard(mockUsersService as any);
  ...
  it('debería arrojar "Email is required" o "Invalid Supabase token" si la data del token viene incompleta', async () => {
    ...
    await expect(guard.canActivate(mockContext)).rejects.toThrow('Email is required');

    mockRequest.user = { email: 'admin@smiab.com' }; // falta sub
    await expect(guard.canActivate(mockContext)).rejects.toThrow('Invalid Supabase token');
  });
  ...
  it('debería inyectar "internal", "areaId" y "unitId" al request.user permitiendo el acceso si todo es válido', async () => {
    ...
    const internalUser = { id: 1, department_id: 'dep-99', unit_id: 'unit-55', region_id: 'reg-01' };
