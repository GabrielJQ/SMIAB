...
export const PrinterHistoryWidget = () => {
    ...
    if (!selectedPrinterId) return (
        <DashboardCard className="flex flex-col items-center justify-center text-slate-300">
            ...
            <p className="text-xl font-bold text-slate-400 uppercase tracking-widest">Monitor de Producción</p>
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] opacity-50 mt-2">Selecciona un equipo de impresión</p>
        </DashboardCard>
    );
    ...
    if (isLoading) return (
        <DashboardCard className="flex items-center justify-center">
            <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-[10px] font-bold text-slate-400 tracking-[0.3em]">CARGANDO HISTORIAL...</p>
            </div>
        </DashboardCard>
    );
                            ...
                            if (active && payload && payload.length) {
                                ...
                                return (
                                    <div className="bg-white/90 backdrop-blur-xl p-4 rounded-2xl shadow-xl border border-white/50 ring-1 ring-slate-100/50">
                                        <p className="text-[10px] uppercase font-bold text-slate-400 mb-2 tracking-wider">{data.fullName || data.name}</p>
